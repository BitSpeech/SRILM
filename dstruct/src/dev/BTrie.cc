...
#ifndef lint
static char Trie_Copyright[] = "Copyright (c) 1995, SRI International.  All Rights Reserved.";
static char Trie_RcsId[] = "@(#)$Header: /home/spot71/srilm/devel/dstruct/src/RCS/Trie.cc,v 1.11 1996/12/08 09:09:54 stolcke Exp $";
#endif
...
Trie<KeyT,DataT>::insertTrie(const KeyT *keys, Boolean &foundP)
{
	...
	Boolean foundThis;
	Trie<KeyT,DataT> *subtrie = sub.insert(keys[0], foundThis);

	/*
	 * If a new node was created we need to initialize the embedded
	 * index.
	 */
        if (!foundThis) {
	    /*
	     * The following should be equivalent, but some compilers don't
	     * allow explicit calling of a constructor, so use new() instead.
	     */
	    //subtrie->sub.TRIE_INDEX_T(0);
...
Trie<KeyT,DataT>::removeTrie(const KeyT *keys, Boolean &foundP)
{
	...
	if (foundP) {
	    /*
	     * Need to call destructor explicitly to avoid freeing
	     */
	    subtrie->sub.~TRIE_INDEX_T();
	    return subtrie;
	} else {
	    return 0;
	}
